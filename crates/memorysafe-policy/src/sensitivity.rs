...
fn looks_like_phone(text: &str) -> bool {
    ...
    digits.len() >= 10 && text.chars().any(|c| matches!(c, '-' | '(' | ')' | '+'))
}
...
mod tests {
    ...
    fn a_token_shape_needs_at_least_twenty_characters() {
        ...
        let short = assess(&candidate_from("token A1b2C3d4E5f6G7h8I9j here", None));
        assert!(!short.categories.contains(&SensitivityCategory::Credential));

        let at_floor = assess(&candidate_from("token A1b2C3d4E5f6G7h8I9j0 here", None));
        assert!(
            at_floor
                .categories
    ...
    fn an_ordinary_long_word_is_not_mistaken_for_a_secret() {
        ...
        let a = assess(&candidate_from(
            "the aforementionedcircumstances were unusual",
            None,
        ));
        assert!(!a.categories.contains(&SensitivityCategory::Credential));
    }
    ...
    fn punctuation_inside_a_long_run_rules_out_the_secret_shape() {
        ...
        let a = assess(&candidate_from(
            "an exclamation!1234567890abcdef mark was unusual",
            None,
        ));
        assert!(!a.categories.contains(&SensitivityCategory::Credential));
    }
    ...
    fn a_phone_number_is_personal() {
        let a = assess(&candidate_from("call them back at (555) 123-4567", None));
        assert!(a.level >= SensitivityLevel::Personal);
        assert!(a.categories.contains(&SensitivityCategory::Pii));
    }
    ...
    fn ten_digits_alone_without_phone_punctuation_is_not_a_phone_number() {
        ...
        let a = assess(&candidate_from("reference id 5551234567 was created", None));
        assert!(!a.categories.contains(&SensitivityCategory::Pii));
    }

    #[test]
    fn phone_punctuation_alone_without_ten_digits_is_not_a_phone_number() {
        let a = assess(&candidate_from("call (555) 123 now", None));
        assert!(!a.categories.contains(&SensitivityCategory::Pii));
    }

    #[test]
    fn nine_digits_falls_just_short_of_the_phone_floor() {
        let a = assess(&candidate_from("dial (555) 123-456 now", None));
        assert!(!a.categories.contains(&SensitivityCategory::Pii));
    }
    ...
    fn an_at_sign_at_the_start_of_a_token_is_not_an_email() {
        let a = assess(&candidate_from("ping @example.com for details", None));
        assert!(!a.categories.contains(&SensitivityCategory::Pii));
    }

    #[test]
    fn a_domain_with_no_dot_is_not_an_email() {
        let a = assess(&candidate_from("reach them at someone@examplecom", None));
        assert!(!a.categories.contains(&SensitivityCategory::Pii));
    }

    #[test]
    fn a_sentence_final_period_does_not_manufacture_an_email() {
        let a = assess(&candidate_from("write to someone@example.com.", None));
        assert!(!a.categories.contains(&SensitivityCategory::Pii));
    }
    ...
    fn a_dot_immediately_before_the_at_sign_does_not_count_as_the_domains_dot() {
        ...
        let a = assess(&candidate_from("write to contact.@localhost now", None));
        assert!(!a.categories.contains(&SensitivityCategory::Pii));
    }
