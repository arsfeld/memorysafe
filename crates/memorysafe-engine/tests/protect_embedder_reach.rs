...
/// regardless of what text is embedded. Counts calls, since a mutation that
/// skips calling the embedder entirely (rather than merely discarding its
/// result) is a distinct failure mode worth being able to detect directly.
struct SpyEmbedder {
    dim: u16,
    id: EmbedderId,
    calls: AtomicUsize,
